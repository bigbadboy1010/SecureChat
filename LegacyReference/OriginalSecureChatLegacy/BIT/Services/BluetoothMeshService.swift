...
final class BluetoothMeshService: NSObject, @unchecked Sendable {

// MARK: - Peripheral safety (avoid CoreBluetooth API MISUSE when disconnected)

private struct PendingPeripheralCommand {
    let peripheralID: UUID
    let action: (CBPeripheral) -> Void
    let enqueuedAt: Date
}

private var pendingPeripheralCommands: [PendingPeripheralCommand] = []
private let pendingPeripheralCommandsMax = 64

private func enqueuePeripheralCommand(_ peripheral: CBPeripheral, action: @escaping (CBPeripheral) -> Void) {
    // Only enqueue for connectable peripherals (we may reconnect); otherwise drop.
    if pendingPeripheralCommands.count >= pendingPeripheralCommandsMax {
        pendingPeripheralCommands.removeFirst()
    }
    pendingPeripheralCommands.append(.init(peripheralID: peripheral.identifier, action: action, enqueuedAt: Date()))
}

private func drainPendingCommands(for peripheral: CBPeripheral) {
    // Drain commands for this peripheral when it becomes connected.
    guard peripheral.state == .connected else { return }
    let now = Date()
    // Drop stale commands (older than 30s)
    let staleCutoff = now.addingTimeInterval(-30)
    pendingPeripheralCommands.removeAll(where: { $0.enqueuedAt < staleCutoff })

    let (matching, rest) = pendingPeripheralCommands.partitioned { $0.peripheralID == peripheral.identifier }
    pendingPeripheralCommands = rest
    for cmd in matching {
        cmd.action(peripheral)
    }
}

private func safeWrite(_ data: Data, to characteristic: CBCharacteristic, on peripheral: CBPeripheral, type: CBCharacteristicWriteType = .withoutResponse) {
    guard peripheral.state == .connected else {
        self.enqueuePeripheralCommand(peripheral) { [weak self] p in
            guard let self else { return }
            guard p.state == .connected else { return }
            // Note: characteristic reference is safe to reuse; if it's invalid, CoreBluetooth will ignore.
            p.writeValue(data, for: characteristic, type: type)
            // Drain any newly queued commands opportunistically.
            self.drainPendingCommands(for: p)
        }
        return
    }
...
private func safeReadValue(for characteristic: CBCharacteristic, on peripheral: CBPeripheral) {
    guard peripheral.state == .connected else {
        self.enqueuePeripheralCommand(peripheral) { [weak self] p in
            guard let self else { return }
            guard p.state == .connected else { return }
...
private func safeSetNotify(_ enabled: Bool, for characteristic: CBCharacteristic, on peripheral: CBPeripheral) {
    guard peripheral.state == .connected else {
        self.enqueuePeripheralCommand(peripheral) { [weak self] p in
            guard let self else { return }
            guard p.state == .connected else { return }
...
private func safeDiscoverServices(_ serviceUUIDs: [CBUUID]?, on peripheral: CBPeripheral) {
    guard peripheral.state == .connected else {
        self.enqueuePeripheralCommand(peripheral) { [weak self] p in
            guard let self else { return }
            guard p.state == .connected else { return }
...
private func safeDiscoverCharacteristics(_ characteristicUUIDs: [CBUUID]?, for service: CBService, on peripheral: CBPeripheral) {
    guard peripheral.state == .connected else {
        self.enqueuePeripheralCommand(peripheral) { [weak self] p in
            guard let self else { return }
            guard p.state == .connected else { return }
    ...
    private let messageQueue = DispatchQueue(label: "schat.messageQueue", attributes: .concurrent)
    ...
    private lazy var batteryOptimizer = BatteryOptimizer.shared
    ...
    private var adaptiveTTL: UInt8 {
        // Base TTL by estimated network size
        let networkSize = estimatedNetworkSize
        let base: UInt8
        if networkSize <= 20 {
            base = 6  // Small networks: max distance
        } else if networkSize <= 50 {
            base = 5  // Medium networks: still good reach
        } else if networkSize <= 100 {
            base = 4  // Large networks: reasonable reach
        } else {
            base = 3  // Very large networks: minimum viable
        }

        // Bandwidth policy adjustment (user setting)
        let mode: BandwidthMode = (delegate as? ChatViewModel)?.settings.bandwidthMode ?? .normal
        switch mode {
        case .low:
            return max(2, base - 1)
        case .normal:
                ...
                if messages.count == 1 {
                    ...
                    if destination == nil {
                        ...
                        if let data = messages[0].toBinaryData() {
                            self.safeWrite(data, to: characteristic, on: peripheral, type: .withoutResponse)
                        }
                    }
                } else {
    ...
    override init() {
        ...
        bloomFilterResetTimer = Timer.scheduledTimer(withTimeInterval: 300.0, repeats: true) { [weak self] _ in
            self?.messageQueue.async(flags: .barrier) { [weak self] in
                ...
                let networkSize = strongSelf.estimatedNetworkSize
                strongSelf.messageBloomFilter = OptimizedBloomFilter.adaptive(for: networkSize)

                // Clear other duplicate detection sets
                strongSelf.processedMessages.removeAll()
                strongSelf.processedKeyExchanges.removeAll()

                print("[BloomFilter] Reset with network size: \(networkSize), memory: \(strongSelf.messageBloomFilter.memorySizeBytes) bytes")
            }
        }
    ...
    func sendMessage(_ content: String, mentions: [String] = [], channel: String? = nil, to recipientID: String? = nil) {
        ...
        // Optional: block sending to quarantined peers (local mitigation)
        if let rid = recipientID, TrafficMitigationService.shared.isPeerQuarantined(rid) {
            Task { @MainActor in
                SecurityInsightsStore.shared.record(.init(kind: .rateLimited, peerID: rid, channel: channel, detail: "outgoing-quarantined"))
            }
            return
        }
        Task { @MainActor in

            SecurityInsightsStore.shared.record(.init(kind: .messageSent, peerID: recipientID, channel: channel))
    ...
    func sendPrivateMessage(_ content: String, to recipientPeerID: String, recipientNickname: String, messageID: String? = nil) {
        ...
        Task { @MainActor in
            ...
            SecurityInsightsStore.shared.record(.init(kind: .messageSent, peerID: recipientPeerID, channel: nil))
    ...
    func sendDeliveryAck(_ ack: DeliveryAck, to recipientID: String) {
        guard (delegate as? ChatViewModel)?.settings.deliveryReceiptsEnabled ?? true else { return }
        messageQueue.async { [weak self] in
            guard let self = self else { return }
    ...
    func sendReadReceipt(_ receipt: ReadReceipt, to recipientID: String) {
        guard (delegate as? ChatViewModel)?.settings.readReceiptsEnabled ?? true else { return }
        messageQueue.async { [weak self] in
            guard let self = self else { return }
        ...
        if let data = packet.toBinaryData() {
            ...
            if let peripheral = connectedPeripherals[peerID],
               ...
               let characteristic = peripheral.services?.first(where: { $0.uuid == BluetoothMeshService.serviceUUID })?.characteristics?.first(where: { $0.uuid == BluetoothMeshService.characteristicUUID }) {
                ...
                self.safeWrite(data, to: characteristic, on: peripheral, type: writeType)
            } else {
            }
        } else {
        }
        ...
        messageQueue.async(flags: .barrier) { [weak self] in
            ...
            if timeDiff > 300000 { // 5 minutes in milliseconds
                ...
                Task { @MainActor in
                    ...
                    SecurityInsightsStore.shared.record(.init(kind: .replayDropped, peerID: peerID, channel: nil, detail: "timestamp-window"))
...
// Adaptive mitigation: rate-limit + quarantine incoming peers before heavier work
if !TrafficMitigationService.shared.allowIncoming(peerID: senderID) {
            Task { @MainActor in
                UserVisibleSecurityBanner.shared.show(title: "Rate Limit", message: "Eingehende Nachrichten wurden gedrosselt (Peer: \(senderID)).")
            }
            return
        }
            ...
            if let recipientID = packet.recipientID {
                if recipientID == SpecialRecipients.broadcast {
                    ...
                    if let signature = packet.signature {
                        let isValid = encryptionService.verify(signature, for: packet.payload, from: senderID)
                        if !isValid {
...
// Local abuse/spam classifier (optional). Never leaves device.
let decision = TrafficMitigationService.shared.evaluateDecryptedContent(finalContent, peerID: senderID, channel: message.channel)
...
case .warn:
    Task { @MainActor in
        SecurityInsightsStore.shared.record(.init(kind: .rateLimited, peerID: senderID, channel: message.channel, detail: "suspicious:\(String(format: "%.2f", decision.probability))"))
        UserVisibleSecurityBanner.shared.show(title: "Verdächtige Nachricht", message: "Diese Nachricht wirkt auffällig (Score: \(Int(decision.probability * 100))%).")
    }

case .drop:
    Task { @MainActor in
        SecurityInsightsStore.shared.record(.init(kind: .rateLimited, peerID: senderID, channel: message.channel, detail: "dropped-spam:\(String(format: "%.2f", decision.probability))"))
        UserVisibleSecurityBanner.shared.show(title: "Nachricht blockiert", message: "Eine potenziell schädliche Nachricht wurde blockiert (Score: \(Int(decision.probability * 100))%).")
    }
    return

case .quarantine:
    Task { @MainActor in
        SecurityInsightsStore.shared.record(.init(kind: .rateLimited, peerID: senderID, channel: message.channel, detail: "quarantine-spam:\(String(format: "%.2f", decision.probability))"))
        UserVisibleSecurityBanner.shared.show(title: "Peer quarantined", message: "Peer wurde temporär quarantined (Score: \(Int(decision.probability * 100))%).")
    }
    return
}
let messageWithPeerID = BitchatMessage(
                        ...
                        self.lastMessageFromPeer[senderID] = Date()
                        DispatchQueue.main.async {
                        Task { @MainActor in

                            SecurityInsightsStore.shared.record(.init(kind: .messageReceived, peerID: senderID, channel: message.channel, detail: message.isPrivate ? "private" : "channel"))
                    ...
                    if relayPacket.ttl > 0 {
                        ...
                        let shouldRelay = relayPacket.ttl >= 4 || self.activePeers.count <= 3 || Double.random(in: 0...1) < relayProb
                        if shouldRelay {
                ...
                } else if let recipientIDString = String(data: recipientID.trimmingNullBytes(), encoding: .utf8), recipientIDString == myPeerID {
                    ...
                    do {
                        let decryptedPadded = try encryptionService.decryptPrivate(packet.payload, from: senderID)
                    ...
                    if let message = BitchatMessage.fromBinaryPayload(decryptedPayload) {
                        ...
                        if let lastReceived = self.receivedMessageTimestamps[messageKey] {
                            ...
                            if timeSinceLastReceived < 5.0 {
                                // duplicate within 5s; ignore silently
                            }
                        }
                    ...
                    var relayPacket = packet
                    relayPacket.ttl -= 1
                    if let recipientIDString = String(data: recipientID.trimmingNullBytes(), encoding: .utf8), let publicKeyData = self.encryptionService.getPeerIdentityKey(recipientIDString) {
                        let fingerprint = self.getPublicKeyFingerprint(publicKeyData)
                    ...
                    let relayProb = min(self.adaptiveRelayProbability + 0.15, 1.0)
                    let shouldRelay = relayPacket.ttl >= 4 || self.activePeers.count <= 3 || Double.random(in: 0...1) < relayProb
                    if shouldRelay {
                        let delay = Double.random(in: minMessageDelay...maxMessageDelay)
                        DispatchQueue.main.asyncAfter(deadline: .now() + delay) { [weak self] in
        ...
        case .deliveryAck:
            ...
            if let recipientIDData = packet.recipientID,
               ...
               recipientID == myPeerID {
                ...
                if let senderID = String(data: packet.senderID.trimmingNullBytes(), encoding: .utf8) {
                    do {
                        let decryptedData = try encryptionService.decryptPrivate(packet.payload, from: senderID)
...
extension BluetoothMeshService: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        self.drainPendingCommands(for: peripheral)
peripheral.delegate = self
        self.safeDiscoverServices([BluetoothMeshService.serviceUUID], on: peripheral)
...
extension BluetoothMeshService: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        for service in services {
            self.safeDiscoverCharacteristics([BluetoothMeshService.characteristicUUID], for: service, on: peripheral)
        }
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in characteristics {
            if characteristic.uuid == BluetoothMeshService.characteristicUUID {
                self.safeSetNotify(true, for: characteristic, on: peripheral)
...
private static func randomUInt64() -> UInt64 {
    var value: UInt64 = 0
    let status = withUnsafeMutableBytes(of: &value) { buf in
    ...
    // fallback
    return UInt64(Date().timeIntervalSince1970) ^ UInt64.random(in: UInt64.min...UInt64.max)
}
...
private extension Array {
    func partitioned(_ predicate: (Element) -> Bool) -> ([Element], [Element]) {
        var yes: [Element] = []
        var no: [Element] = []
        yes.reserveCapacity(count)
        no.reserveCapacity(count)
        for el in self {
            if predicate(el) { yes.append(el) } else { no.append(el) }
        }
        return (yes, no)
    }
}
