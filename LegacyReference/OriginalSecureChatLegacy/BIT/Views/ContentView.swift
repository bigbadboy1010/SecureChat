...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .alert(item: $viewModel.safetyAlert) { alert in
                ...
                HStack(spacing: 4) {
                    ...
                    HStack(spacing: 0) {
                        ...
                        TextField("nickname", text: $viewModel.nickname)
                            ...
                            .frame(maxWidth: 100)
                            .foregroundColor(textColor)
                            .onChangeCompat(of: viewModel.nickname) {
    ...
    private var messagesView: some View {
        ScrollViewReader { proxy in
            ...
            .onChange(of: viewModel.messages) { oldMessages, newMessages in
                if viewModel.selectedPrivateChatPeer == nil && !newMessages.isEmpty {
                    withAnimation {
                        proxy.scrollTo(newMessages.last?.id, anchor: .bottom)
                    }
                }
            }
            .onChange(of: viewModel.privateChats) { oldChats, newChats in
                if let peerID = viewModel.selectedPrivateChatPeer,
                   let messages = newChats[peerID],
                   !messages.isEmpty {
                    withAnimation {
            ...
            .onChange(of: viewModel.selectedPrivateChatPeer) { oldPeerID, newPeerID in
