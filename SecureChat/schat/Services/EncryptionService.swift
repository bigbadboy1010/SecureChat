...
class EncryptionService {
    ...
    // Thread safety
    private let cryptoQueue = DispatchQueue(label: "chat.schat.crypto", attributes: .concurrent)
    ...
    init() {
        ...
        if let identityData = UserDefaults.standard.data(forKey: "schat.identityKey"),
           let loadedKey = try? Curve25519.Signing.PrivateKey(rawRepresentation: identityData) {
            ...
            UserDefaults.standard.set(identityKey.rawRepresentation, forKey: "schat.identityKey")
        }
    ...
    func clearPersistentIdentity() {
        UserDefaults.standard.removeObject(forKey: "schat.identityKey")
