...
@Published var safetyAlert: SafetyAlert? = nil
...
private var blockedUsers: Set<String> = []  // Stores public key fingerprints of blocked users
    ...
    @Published var settings: ChatSettings = ChatSettings.load() {
        didSet { settings.save() }
    }

    @Published private(set) var verifiedPeers: Set<String> = []

    private var encryptionService: EncryptionService {
        meshService.encryptionService
    }

    var myIdentityFingerprint: String {
        IdentityVerificationService.shared.myIdentityFingerprint(from: encryptionService)
    }

    func peerIdentityFingerprint(peerID: String) -> String? {
        IdentityVerificationService.shared.peerIdentityFingerprint(from: encryptionService, peerID: peerID)
    }

    func isPeerVerified(_ peerID: String) -> Bool {
        verifiedPeers.contains(peerID)
    }

    @discardableResult
    func verifyPeer(_ peerID: String, expectedFingerprint: String) -> Bool {
        let ok = IdentityVerificationService.shared.verifyPeer(peerID: peerID, expectedFingerprint: expectedFingerprint, encryptionService: encryptionService)
        if ok {
            refreshVerifiedPeers()
        }
        return ok
    }

    func unverifyPeer(_ peerID: String) {
        IdentityVerificationService.shared.unverifyPeer(peerID: peerID)
        refreshVerifiedPeers()
    }

    private func refreshVerifiedPeers() {
        // Keep a cheap in-memory set for UI usage.
        var set = Set<String>()
        for p in connectedPeers {
            if IdentityVerificationService.shared.isPeerVerified(p) { set.insert(p) }
        }
        verifiedPeers = set
    }
    ...
    func channelInviteString(for channelTag: String) -> String? {
        let normalized = channelTag.hasPrefix("#") ? channelTag : "#\(channelTag)"
        

ensureUnprotectedChannelKey(normalized)
let token = newInviteToken()
// Default: 10 minutes validity for invites
let expires = Int(Date().addingTimeInterval(10 * 60).timeIntervalSince1970)

var channelKeyB64: String? = nil
var wrappedB64: String? = nil
var saltB64: String? = nil

if let key = channelKeys[normalized], !passwordProtectedChannels.contains(normalized) {
    let raw = key.withUnsafeBytes { Data($0) }
    if let wrapped = wrapChannelKeyForInvite(rawKey: raw, passphrase: invitePassphrase, channelTag: normalized) {
        wrappedB64 = wrapped.wrappedB64
        saltB64 = wrapped.saltB64
    } else {
        channelKeyB64 = raw.base64EncodedString()
    }
}

let payload = ChannelInvitePayload(
    channelTag: normalized,
    isProtected: passwordProtectedChannels.contains(normalized),
    creatorPeerID: channelCreators[normalized],
    keyCommitment: channelKeyCommitments[normalized],
    channelKeyB64: channelKeyB64,
    wrappedChannelKeyB64: wrappedB64,
    passphraseSaltB64: saltB64,
    inviteToken: token,
    expiresAtUnix: expires,
    retentionEnabled: retentionEnabledChannels.contains(normalized)
)
return payload.encodeString()
    }

    @discardableResult
    func applyInviteString(_ scanned: String) -> Bool {
        // The QR contains a base64(JSON) payload.
        guard let payload = ChannelInvitePayload.decodeString(scanned) else { return false }
        guard payload.version == ChannelInvitePayload.currentVersion else { return false }

        
if let expires = payload.expiresAtUnix {
...
if let token = payload.inviteToken {
    if isInviteTokenUsed(token) {
        print("[Invite] Replay blocked")
        return false
    }
    markInviteTokenUsed(token)
}

let channel = payload.channelTag.hasPrefix("#") ? payload.channelTag : "#\(payload.channelTag)"

if !payload.isProtected {
    if let wrapped = payload.wrappedChannelKeyB64,
       let salt = payload.passphraseSaltB64 {
        let raw = unwrapChannelKeyFromInvite(wrappedB64: wrapped, saltB64: salt, passphrase: invitePassphrase, channelTag: channel)
        guard let raw, raw.count == 32 else {
            self.lastInviteError = "Invite requires passphrase (oder Passphrase falsch)."
            return false
        }
        let key = SymmetricKey(data: raw)
        self.channelKeys[channel] = key
        let commit = SHA256.hash(data: raw).map { String(format: "%02x", $0) }.joined()
        self.channelKeyCommitments[channel] = commit
    } else if let keyB64 = payload.channelKeyB64,
        ...
        self.ensureUnprotectedChannelKey(channel)
    }
}
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if payload.isProtected {
                ...
                if let commitment = payload.keyCommitment {
                    self.channelKeyCommitments[channel] = commitment
                }
                if let creator = payload.creatorPeerID {
                    self.channelCreators[channel] = creator
                }
            }

            if payload.retentionEnabled {
                self.retentionEnabledChannels.insert(channel)
            }

            _ = self.joinChannel(channel)
        }
    ...
    private let nicknameKey = "schat.nickname"
    private let favoritesKey = "schat.favorites"
    private let joinedChannelsKey = "schat.joinedChannels"
    private let passwordProtectedChannelsKey = "schat.passwordProtectedChannels"
    private let channelCreatorsKey = "schat.channelCreators"
    // private let channelPasswordsKey = "schat.channelPasswords" // Now using Keychain
    private let channelKeyCommitmentsKey = "schat.channelKeyCommitments"
    private let retentionEnabledChannelsKey = "schat.retentionEnabledChannels"
    private let blockedUsersKey = "schat.blockedUsers"
...
// Invite hardening (Step 6): optional passphrase to wrap embedded channel keys in QR invites
@Published var invitePassphrase: String = ""
@Published var lastInviteError: String? = nil

// One-time invite replay protection (persisted locally)
@AppStorage("bit.usedInviteTokensV1") private var usedInviteTokensJson: String = "[]"
private var usedInviteTokens: Set<String> = []

private func loadUsedInviteTokensIfNeeded() {
    if !usedInviteTokens.isEmpty { return }
    guard let data = usedInviteTokensJson.data(using: .utf8),
          let arr = try? JSONDecoder().decode([String].self, from: data) else { return }
    usedInviteTokens = Set(arr)
}

private func markInviteTokenUsed(_ token: String) {
    loadUsedInviteTokensIfNeeded()
    usedInviteTokens.insert(token)
    let arr = Array(usedInviteTokens.prefix(5000))
    if let data = try? JSONEncoder().encode(arr),
       let s = String(data: data, encoding: .utf8) {
        usedInviteTokensJson = s
    }
}

private func isInviteTokenUsed(_ token: String) -> Bool {
    loadUsedInviteTokensIfNeeded()
    return usedInviteTokens.contains(token)
}

private func newInviteToken() -> String {
...
// Replay protection for channel messages (Step 6): cache recent msgIds per (channel|sender)
private var channelReplayCache: [String: Set<UInt64>] = [:]

private func isChannelReplay(channel: String, senderPeerID: String, msgId: UInt64) -> Bool {
    let k = "\(channel)|\(senderPeerID)"
    var set = channelReplayCache[k] ?? Set<UInt64>()
    if set.contains(msgId) { return true }
    set.insert(msgId)
    if set.count > 512 {
        set = Set(set.suffix(256))
    }
    channelReplayCache[k] = set
    return false
}

// Passphrase wrap (Step 6): wraps the embedded channel key in the invite using MemoryHardKDF + AES-GCM + AAD.
private func wrapChannelKeyForInvite(rawKey: Data, passphrase: String, channelTag: String) -> (wrappedB64: String, saltB64: String)? {
    guard !passphrase.isEmpty else { return nil }

    var salt = Data(count: 16)
    let st = salt.withUnsafeMutableBytes { buf in
        SecRandomCopyBytes(kSecRandomDefault, 16, buf.baseAddress!)
    }
    guard st == errSecSuccess else { return nil }

    guard let wrapKey = try? MemoryHardKDF.deriveKey(
        password: passphrase,
        salt: salt + (channelTag.data(using: .utf8) ?? Data()),
        memoryKiB: 4096,
        rounds: 3
    ) else { return nil }
    ...
    do {
        ...
        let aad = ("bit-invite-wrap|" + channelTag).data(using: .utf8) ?? Data()
        let sealed = try AES.GCM.seal(rawKey, using: wrapKey, nonce: nonce, authenticating: aad)
...
private func unwrapChannelKeyFromInvite(wrappedB64: String, saltB64: String, passphrase: String, channelTag: String) -> Data? {
    guard !passphrase.isEmpty,
          let salt = Data(base64Encoded: saltB64),
          let combined = Data(base64Encoded: wrappedB64) else { return nil }
...
NotificationCenter.default.addObserver(forName: .bitPeerIdentityChanged, object: nil, queue: .main) { [weak self] note in
    guard let self else { return }
    let peerID = (note.userInfo?["peerID"] as? String) ?? "unknown"
    let old = (note.userInfo?["old"] as? String) ?? "unknown"
    let new = (note.userInfo?["new"] as? String) ?? "unknown"
    self.safetyAlert = SafetyAlert(
        title: "Safety number changed",
        message: "Der Identity-Fingerprint von \(peerID) hat sich geändert.\n\nAlt: \(old)\nNeu: \(new)\n\nVerifiziere den Peer erneut (QR/Fingerprint), bevor du vertrauliche Daten sendest."
    )
}
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            if self.connectedPeers.isEmpty && self.messages.isEmpty {
                let welcomeMessage = BitchatMessage(
                    sender: "system",
                    content: "get people around you to download schat…and chat with them here!",
                    timestamp: Date(),
                    isRelay: false
                )
                    ...
                    if let channelMsgs = channelMessages[channelTag], !channelMsgs.isEmpty {
                        ...
                        if let encryptedMsg = encryptedMessages.first,
                           let encryptedData = encryptedMsg.encryptedContent {
                            let testDecrypted = decryptChannelMessage(encryptedData, channel: channelTag, senderPeerID: encryptedMsg.senderPeerID, testKey: key)
    ...
    private func deriveChannelKey(from password: String, channelName: String) -> SymmetricKey {
        // Memory-hard KDF (CryptoKit-only) to raise offline cracking cost.
        let salt = channelName.data(using: .utf8) ?? Data()
        return (try? MemoryHardKDF.deriveKey(password: password, salt: salt, memoryKiB: 4096, rounds: 3)) ?? SymmetricKey(data: SHA256.hash(data: salt))
    }
    ...
    func sendMessage(_ content: String) {
        ...
        if let selectedPeer = selectedPrivateChatPeer {
            ...
            if let channel = messageChannel {
                ...
                if retentionEnabledChannels.contains(channel) {
                    MessageRetentionService.shared.saveMessage(message, in: channel)
                }
    ...
    func formatMessageAsText(_ message: BitchatMessage, colorScheme: ColorScheme) -> AttributedString {
        ...
        if message.sender != "system" {
            ...
            if message.content.contains("get people around you to download schat") {
...
extension ChatViewModel: BitchatDelegate {
    ...
    func decryptChannelMessage(_ encryptedContent: Data, channel: String, senderPeerID: String? = nil, testKey: SymmetricKey? = nil) -> String? {
    let key = testKey ?? channelKeys[channel]
    guard let key = key else { return nil }

    // Sender-Keys: decryption is per sender. We bind to senderPeerID when available.
    let sender = senderPeerID ?? "unknown"
    do {
        if let hdr = ChannelSenderKeyCrypto.parseHeader(packet: encryptedContent) {
            if isChannelReplay(channel: channel, senderPeerID: sender, msgId: hdr.msgId) { return nil }
        }
        let decryptedData = try ChannelSenderKeyCrypto.decrypt(packet: encryptedContent, channelKey: key, senderPeerID: sender)
            ...
            if joinedChannels.contains(channel) {
                ...
                if message.isEncrypted && channelKeys[channel] == nil {
                    ...
                    if let encryptedData = message.encryptedContent {
                        if let decryptedContent = decryptChannelMessage(encryptedData, channel: channel, senderPeerID: message.senderPeerID) {
                ...
                // Save message if channel has retention enabled
                if retentionEnabledChannels.contains(channel) {
                    MessageRetentionService.shared.saveMessage(messageToAdd, in: channel)
                }
...
// MARK: - Channel Keys (Sender Keys v2)

    private func ensureUnprotectedChannelKey(_ channelTag: String) {
    let normalized = channelTag.hasPrefix("#") ? channelTag : "#\(channelTag)"
    guard !passwordProtectedChannels.contains(normalized) else { return }
    if channelKeys[normalized] != nil { return }

    // Create a random 32-byte key for this channel and store it.
    ...
    if ok != errSecSuccess {
        ...
        raw = Data(SHA256.hash(data: normalized.data(using: .utf8) ?? Data()))
    }
    let key = SymmetricKey(data: raw)
    channelKeys[normalized] = key

    // Commitment: SHA256(keyRaw)
    let commit = SHA256.hash(data: raw).map { String(format: "%02x", $0) }.joined()
    channelKeyCommitments[normalized] = commit

    // Persist (optional): reuse existing persistence if present; otherwise keep in-memory.
}

    private func channelKeyBase64IfAvailable(_ channelTag: String) -> String? {
    let normalized = channelTag.hasPrefix("#") ? channelTag : "#\(channelTag)"
    guard !passwordProtectedChannels.contains(normalized) else { return nil }
    guard let key = channelKeys[normalized] else { return nil }
    let raw = key.withUnsafeBytes { Data($0) }
    return raw.base64EncodedString()
}
...
// One-time invite replay protection (in-memory + persisted)
