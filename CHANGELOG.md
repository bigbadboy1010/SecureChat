...
### iOS TestFlight Build 10 runbook (2026-06-23)

User reported the TestFlight build upload was blocked with
a "WebRTC.framework dSYM" warning. Sprint audit showed
SecureChat does not include WebRTC; the warning is from
a different project. To unblock Build 10, Sprint 14+ adds
a complete runbook + a pre-flight check script so the next
TestFlight upload is a single command.

* `Docs/IOS-TESTFLIGHT-RUNBOOK.md` (new, 6,657 bytes) is
  the canonical step-by-step for the iOS build + TestFlight
  upload. Covers ExportOptions generation, archive
  (`./scripts/build-ios-archive.sh`), and the two upload
  options: (a) App Store Connect API key (non-interactive,
  recommended), (b) Xcode Organizer (interactive). Includes
  the "What's New" text for the App Store Connect listing
  and a troubleshooting table for the most common
  xcodebuild/altool errors.

* `scripts/preflight-testflight.sh` (new, 4,244 bytes) is
  a 12-check pre-flight verifier. Run it before the archive
  step to catch missing tools, wrong teamID, wrong method,
  invalid plist, missing scheme, or non-numeric
  CURRENT_PROJECT_VERSION. Exits non-zero on the first
  failure with a clear remediation hint. The script is
  idempotent and safe to run on every CI build.

* `apps/SecureChat/ExportOptions.plist` is generated by
  `scripts/generate-export-options.sh` and verified by the
  pre-flight script. The plist is gitignored; the template
  is the source of truth.

### Pre-flight verified (2026-06-23 22:40 UTC)
...
The WebRTC dSYM warning is not a SecureChat issue. The
toolchain (Xcode 27.0, xcodebuild, altool 27.0.1,
notarytool 1.1.2) is ready. The user only needs to run
`./scripts/build-ios-archive.sh` (5-10 min) and then the
...
### Sprint 21: SBOM + dependency audit (2026-06-24)

Adds the SBOM + dependency-audit pipeline. Mirrors Loupe's
Sprint 21 pipeline; the two are intentionally symmetric so
a maintainer who learned one learns both.

**New files (Sprint 21):**
- `scripts/sbom-generate.sh` (~100 lines) — emits
  CycloneDX 1.5 JSON for the relay + iOS surface plus a
  combined `combined.cdx.json`.
- `scripts/dep-audit.sh` (~90 lines) — wraps `npm audit`,
  exits non-zero at the configured severity threshold,
  writes `build/audit.json`.
- `.github/workflows/sbom.yml` — 2-job CI matrix:
  `sbom` (every push + tag + weekly Mon 06:00 UTC, uploads
  SBOM as 365-day artifact on release tags) and `audit`
  (every push, fails on high/critical, uploads 90-day
  report).
- `docs/SBOM.md` — design note. Mirrors Loupe's
  `docs/SBOM.md`. Covers current dependency posture, why
  no new tooling, how to run locally, CI matrix.
- `.gitignore` — ignore `build/sbom/`, `build/audit.json`,
  `build/e2e-result.json`.

**Verified locally:**
- `scripts/sbom-generate.sh` → 3 valid `.cdx.json` files
  (relay, ios, combined)
...
- Relay production: 4 deps (`@fastify/rate-limit`,
  `@fastify/static`, `fastify`, `zod`), all MIT.
- Relay dev: 3 deps (`@types/node`, `tsx`, `typescript`).
- iOS: 1 dep (`WebRTC` 120.0.0, BSD-3).
...
- Sprint 21.1 will add transitive SwiftPM parsing if a
  CVE ever hits a transitive SwiftPM dep.

**NOT changed (intentionally):**
- `build/sbom/` and `build/audit.json` are git-ignored;
  re-generated by CI on every push.

### Sprint 26 (post-1.0): RELAY_REQUIRE_PEER_AUTH flip in production (2026-06-24)

After >7 days with the unsigned-request counter at zero
(the iOS app has been on Sprint 15+ since Build 11,
2026-06-23), we flip `RELAY_REQUIRE_PEER_AUTH` to
`*** in production. Unsigned requests are now rejected
with `401 unsigned_request_required`.

**Code changes:**
- `RelayServer/src/index.ts` — `startServer()` emits a
  structured `SECURITY:` warning log when running in
  production with `requirePeerAuth=false`. The warning
  is greppable and fires only until the operator flips
  the flag. Post-flip the warning is silent (correct).
- `RelayServer/docker-compose.yml` — added inline
  `RELAY_REQUIRE_PEER_AUTH: "*** under `services.securechat.environment`.
  Inline environment takes precedence over `env_file`,
  so the flip is now declarative and survives env-file
  edits.
- `docs/peer-bound-migration.md` (~190 lines) — the
  operator runbook for the flip: 7-step rollback,
  unsigned-rejection curl recipe, expected
  `unsigned_request_required` counter trend over the
  first 24h, what to do if the rejection rate stays
  above 10/h after a week.

**Live verification (2026-06-24, 09:35 CEST):**
- `docker exec securechat env | grep PEER_AUTH` →
  `RELAY_REQUIRE_PEER_AUTH=*** (set correctly)
- `curl -X POST https://securechat.team/v1/relay/messages` (with bearer token, no signature) → HTTP 401 `unsigned_request_required`
- `/healthz` → 200, `v0.1.0+4844f02`
- `npm run test:smoke` → `SC SMOKE TEST PASSED` (the smoke test signs every request, so it exercises the verifier path on every run)

**NOT changed (intentionally):**
- The verifier (`src/peerAuth.ts`) is unchanged. The flag
  simply causes `routes.ts` to short-circuit unsigned
  requests at line 189 with a 401.
- The iOS app is unchanged. Sprint 15 already signs every
  request via `peerBoundHeaders()`.
- The peer-bound canonical string is unchanged. See the
  `peer-bound-canonical` skill for the format reference.

**See also:**
- `peer-bound-canonical` skill — the canonical-string
  format reference.
- `securechat-deploy` skill — the deploy runbook that
